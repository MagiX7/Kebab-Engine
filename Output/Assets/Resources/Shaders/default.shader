...
struct Material {
	...
	vec3 specular;
	float shininess;
	int gammaCorrection; // It is a boolean
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
	...
	float attenuation = 1.0 / (light.constant + light.lin * distance + 0.5 * (distance * distance));
...
void main()
{
	...
	vec3 finalColor = pow(result, vec3(1.0 / material.gammaCorrectionAmount)) * material.gammaCorrection;
