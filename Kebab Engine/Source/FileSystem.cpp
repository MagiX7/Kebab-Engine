...
FileSystem::FileSystem(const char* game_path)
{
	...
	// Dump list of paths
	LOG_CONSOLE("FileSystem Operations base is [%s] plus:", GetBasePath());
	LOG_CONSOLE(GetReadPaths());

	// enable us to write in the game's dir area
	if(PHYSFS_setWriteDir(".") == 0)
		LOG_CONSOLE("File System error while creating write dir: %s\n", PHYSFS_getLastError());
...
bool FileSystem::Init(JSON_Object* root)
{
...
bool FileSystem::CopyFromOutsideFS(const char * full_path, const char * destination)
{
	...
	if (source && dest)
	{
		...
		LOG_CONSOLE("File System copied file [%s] to [%s]", full_path, destination);
	}
	else
		LOG_CONSOLE("File System error while copy from [%s] to [%s]", full_path, destination);
...
bool FileSystem::Copy(const char * source, const char * destination)
{
	...
	if (src && dst)
	{
		...
		LOG_CONSOLE("File System copied file [%s] to [%s]", source, destination);
	}
	else
		LOG_CONSOLE("File System error while copy from [%s] to [%s]", source, destination);
...
uint FileSystem::Save(const char* file, const void* buffer, unsigned int size, bool append) const
{
	...
	if(fs_file != nullptr)
	{
		...
		if (written != size)
		{
			LOG_CONSOLE("File System error while writing to file %s: %s", file, PHYSFS_getLastError());
		}
		else
		{
			if (append == true)
			{
				LOG_CONSOLE("Added %u data to [%s%s]", size, PHYSFS_getWriteDir(), file);
			}
			//else if(overwrite == true)
				//LOG_CONSOLE("File [%s%s] overwritten with %u bytes", PHYSFS_getWriteDir(), file, size);
			else if(overwrite == false)
				LOG_CONSOLE("New file created [%s%s] of %u bytes", PHYSFS_getWriteDir(), file, size);

			ret = written;
		}

		if(PHYSFS_close(fs_file) == 0)
			LOG_CONSOLE("File System error while closing file %s: %s", file, PHYSFS_getLastError());
	}
	else
		LOG_CONSOLE("File System error while opening file %s: %s", file, PHYSFS_getLastError());
...
bool FileSystem::Remove(const char * file)
{
	...
	if (file != nullptr)
	{
		...
		{
			LOG_CONSOLE("File deleted: [%s]", file);
			ret = true;
		}
		else
			LOG_CONSOLE("File System error while trying to delete [%s]: ", file, PHYSFS_getLastError());
	}
...
size_t AssimpWrite(aiFile* file, const char* data, size_t size, size_t chunks)
{
	...
	if(ret == -1)
		LOG_CONSOLE("File System error while WRITE via assimp: %s", PHYSFS_getLastError());
...
size_t AssimpRead(aiFile* file, char* data, size_t size, size_t chunks)
{
	...
	if(ret == -1)
		LOG_CONSOLE("File System error while READ via assimp: %s", PHYSFS_getLastError());
...
size_t AssimpTell(aiFile* file)
{
	...
	if(ret == -1)
		LOG_CONSOLE("File System error while TELL via assimp: %s", PHYSFS_getLastError());
...
size_t AssimpSize(aiFile* file)
{
	...
	if(ret == -1)
		LOG_CONSOLE("File System error while SIZE via assimp: %s", PHYSFS_getLastError());
...
void AssimpFlush(aiFile* file)
{
	if(PHYSFS_flush((PHYSFS_File*)file->UserData) == 0)
		LOG_CONSOLE("File System error while FLUSH via assimp: %s", PHYSFS_getLastError());
}
...
aiReturn AssimpSeek(aiFile* file, size_t pos, aiOrigin from)
{
	...
	if(res == 0)
		LOG_CONSOLE("File System error while SEEK via assimp: %s", PHYSFS_getLastError());
...
void AssimpClose(aiFileIO* io, aiFile* file)
{
	if (PHYSFS_close((PHYSFS_File*)file->UserData) == 0)
		LOG_CONSOLE("File System error while CLOSE via assimp: %s", PHYSFS_getLastError());
}
...
void CALLBACK BassClose(void* file)
{
	if (PHYSFS_close((PHYSFS_File*)file) == 0)
		LOG_CONSOLE("File System error while CLOSE via bass: %s", PHYSFS_getLastError());
}
...
//	if(ret == -1)
//		LOG_CONSOLE("File System error while SIZE via bass: %s", PHYSFS_getLastError());
//
//	return (QWORD) ret;
//}
//
...
//	if(ret == -1)
//		LOG_CONSOLE("File System error while READ via bass: %s", PHYSFS_getLastError());
//
//	return (DWORD) ret;
//}
//
...
//	if(res == 0)
//		LOG_CONSOLE("File System error while SEEK via bass: %s", PHYSFS_getLastError());
//
//	return (BOOL) res;
//}